...
typedef struct {
    ...
    XZ_frmbuf_writer_uio_map maps[ MAX_UIO_MAPS ];
} XZ_frmbuf_writer_uio_info;

/***************** Variable Definitions **************************************/
static XZ_frmbuf_writer_uio_info uio_info;
...
static int uio_info_read_name(XZ_frmbuf_writer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XZ_frmbuf_writer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XZ_frmbuf_writer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XZ_frmbuf_writer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XZ_frmbuf_writer_Initialize(XZ_frmbuf_writer *InstancePtr, const char* InstanceName) {
	XZ_frmbuf_writer_uio_info *InfoPtr = &uio_info;
