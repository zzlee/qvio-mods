...
typedef struct {
    ...
    XAximm_test1_uio_map maps[ MAX_UIO_MAPS ];
} XAximm_test1_uio_info;

/***************** Variable Definitions **************************************/
static XAximm_test1_uio_info uio_info;
...
static int uio_info_read_name(XAximm_test1_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XAximm_test1_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XAximm_test1_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XAximm_test1_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XAximm_test1_Initialize(XAximm_test1 *InstancePtr, const char* InstanceName) {
	XAximm_test1_uio_info *InfoPtr = &uio_info;
