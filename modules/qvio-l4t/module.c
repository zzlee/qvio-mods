...
#if 0
	err = qvio_platform_device_register();
	if (err != 0) {
		pr_err("qvio_platform_device_register() failed, err=%d\n", err);
		goto err1;
	}
#endif

	err = qvio_pci_device_register();
	if (err != 0) {
		pr_err("qvio_pci_device_register() failed, err=%d\n", err);
		goto err1;
	}
...
err1:
#if 0
	qvio_platform_device_unregister();
err0:
#endif
	return err;
}
